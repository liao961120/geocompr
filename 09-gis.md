...
\caption[Sliver (left panel) and cleaned (right panel) polygons.]{Sliver polygons colored in blue (left panel). Cleaned polygons (right panel).}(\#fig:sliver-fig)
\end{figure}
...
\caption{SAGA wetness index of Mount Mongón, Peru.}(\#fig:saga-twi)
\end{figure}
...
\caption[Shortest route between 24 cycle hire stations.]{Shortest route (blue line) between 24 cycle hire stations (blue dots) on the OSM street network of London.}(\#fig:grass-mapview)
\end{figure}
...
\caption[Visualization of the output of previous PostGIS commands.]{Visualization of the output of previous PostGIS commands showing the highway (black line), a buffer (light yellow) and three restaurants (light blue points) within the buffer.}(\#fig:postgis)
\end{figure}
